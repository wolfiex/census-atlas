<script>
  ...
  $: scale =
    (breaks[1] - domain[0]) / (domain[1] - domain[0]) < 1 / 20
      ? 0.5
      : (breaks[4] - domain[0]) / (domain[1] - domain[0]) > 19 / 20
      ? 2
      : 1;

  $: formatTick = (d) =>
    d.toFixed(Math.abs(breaks[1] - domain[0]) < 0.1 ? 2 : Math.abs(breaks[1] - domain[0]) < 1 ? 1 : 0);
  ...
  <div class="chart-container">
    ...
    >
      <Svg>
        ...
        <Stack ticks={breaks} muted={sibData ? true : false} {maskId} />
        <Marker
          values={sibData}
          ...
          {key}
        />
        <AxisX gridlines={true} baseline={true} snapTicks={true} ticks={breaks} dyTick={5} {formatTick} />
      </Svg>
    </LayerCake>
  </div>
{/if}
